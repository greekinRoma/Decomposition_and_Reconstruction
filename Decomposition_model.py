...
class DecompositionModel(nn.Module):
    def __init__(self, num_head=16, m=2, n =2, origin_patch_size=4, resize_patch_size=16, origin_embed_dim = 48, resize_embed_dim=48, *args, **kwargs):
        ...
        self.b = nn.Linear(self.origin_embed_dim+ self.origin_embed_dim//self.num_heads//6, self.origin_embed_dim)
        self.q  = nn.Linear(self.resize_embed_dim+ self.origin_embed_dim//self.num_heads//6, self.resize_embed_dim)
        ...
        self.mat_mul = rope(num_heads=1,head_dim=self.origin_embed_dim//self.num_heads//6, rope_theta=10.0)
    ...
    def forward(self, x):
        ...
        xq_freqs_cls, xb_freqs_cls = self.mat_mul.compute_rope( end_x_xq=rw, end_y_xq=rh, end_x_xb=ow, end_y_xb=oh)
        xq_freqs_cls = xq_freqs_cls.reshape(1,rh,rw,-1).expand(self.m*self.n, -1, -1, -1)
        xb_freqs_cls = xb_freqs_cls.reshape(1,oh,ow,-1).expand(self.m*self.n, -1, -1, -1)
        resize_patches = torch.concat([resize_patches,xq_freqs_cls],dim=-1)
        origin_patches = torch.concat([origin_patches,xb_freqs_cls],dim=-1)
