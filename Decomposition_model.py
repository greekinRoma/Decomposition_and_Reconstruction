...
class DecompositionModel(nn.Module):
    ...
    def forward(self, x):
        ...
        resized_patches = (self.resize_patch_embed(resized_x) + self.pos_embed).permute(0,2,3,1).reshape(B, self.reszie * self.reszie, self.embed_dim)
        ...
        # x = q.reshape(B, self.num_heads, self.m, self.reszie//self.m, self.reszie, -1).permute(0, 2, 3, 1, 4).reshape(B, self.reszie, self.reszie, -1)
        ...
        x = (attn @ tmp_b).reshape(B, self.num_heads, self.m, self.n, self.reszie//self.m, self.reszie//self.n, -1).permute(0, 2, 4, 3, 5, 1, 6).reshape(B, self.reszie, self.reszie, self.embed_dim)
