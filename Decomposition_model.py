...
class DecompositionModel(nn.Module):
    def __init__(self, num_head=16, m=2, n =2, origin_patch_size=4, resize_patch_size=16, origin_embed_dim = 48, resize_embed_dim=48, *args, **kwargs):
        ...
        self.q = nn.Parameter(torch.rand(1,self.num_heads,1024 // self.resize_patch_size*1024 // self.resize_patch_size//self.m//self.n,self.resize_embed_dim//self.num_heads))
        self.rw = 1024//self.resize_patch_size//self.n
        self.rh = 1024//self.resize_patch_size//self.m
    def forward(self, x):
        B, C, H, W = x.shape
        ...
        tmp_b = b.clone()
        q = torch.nn.functional.normalize(self.q.expand(self.m*self.n, -1, -1, -1), dim=-1)
        b = torch.nn.functional.normalize(b, dim=-1)

        attn = self.mat_mul.multiply(q=q, b=b, end_x_xq=self.rw, end_y_xq=self.rh, end_x_xb=ow, end_y_xb=oh)
